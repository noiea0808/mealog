 ...
 */
/**
 * ── 버그 3: 일상어를 금칙어에 넣지 않는다 ──────────────────────────────────
 *
 * `무료`·`이벤트`·`할인`·`쿠폰` 은 스팸의 표지가 아니라 그냥 쓰는 말이다.
 * 「무료로 쓰실 수 있어요」·「이벤트 기간에는」 같은 정상 답변이 그대로 막혔다.
 * 이 목록은 **그 말이 들어간 글은 거의 다 스팸**일 때만 값을 한다.
 */
const BANNED_WORDS = [
  { label: '스팸 의심', re: /(광고|홍보|추천인)/i },
