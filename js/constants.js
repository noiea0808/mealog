...
export const DEFAULT_USER_SETTINGS = {
    ...
    tags: {
        /**
         * '어떻게'(조달) — 집 → 가져와서 → 밖 순으로 늘어놓는다. 자주 쓰는 값이 앞에 오게.
         * '편의점'은 집밥·외식·배달 어디에도 안 맞는데 실제로 흔해서 새로 만든 칸이다
         * (도시락·삼각김밥·컵라면).
         *
         * ⚠️ 이 상수만 바꿔서는 기존 사용자에게 아무 변화가 없다 — 관리자 태그 문서
         * (content/defaultTags.mealType)가 이 값을 덮는다(js/db/listeners.js applyAdminTags).
         * 형태 축과 달리 파일럿 게이트도 없어서 관리자 화면에서 저장하는 순간 전원에게 반영된다.
         */
