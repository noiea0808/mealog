 ...
 *   usageDaily 전 구간 + 최근 7일    → usageDaily 미러
 *
 * 남는 서버 읽기는 미러 **동기화 자체**(변경분 + 삭제 감지 count)와 캐시 문서
 * 몇 건뿐이다.
 *
...
import { sharedPhotosMirror, usageDailyMirror } from './collection-mirror.js';
import { mealRowToDocLike } from './dashboard-mirror-model.js';

/**
 * 네 미러를 최신으로 맞추고 재료를 꺼낸다.
 *
 * @param {{force?: boolean}} [options] force 면 네 미러 모두 전체 재구축
 * @returns {Promise<{
 *   mealRows: object[],
 *   mealDocs: object[],
 *   userRows: object[],
 *   sharedDocs: object[],
 *   usageDailyDocs: object[],
 *   serverReads: number,
...
export async function loadDashboardMirrorSource(options = {}) {
    ...
    /**
     * 순차로 돌린다. 넷을 한꺼번에 띄우면 부트스트랩이 겹칠 때 브라우저 커넥션과
    ...
    const usageResult = await usageDailyMirror.ensureSynced(undefined, { force });

    const [mealRows, userRows, sharedDocs, usageDailyDocs] = await Promise.all([
        getAllMealsFromMirror(),
        getAllUserMirrorRows(),
        sharedPhotosMirror.getDocsLike(),
        usageDailyMirror.getDocsLike()
    ]);

    const serverReads =
        (mealsResult?.fetched || 0) +
        (usersResult?.serverReads || 0) +
        (sharedResult?.serverReads || 0) +
        (usageResult?.serverReads || 0);
