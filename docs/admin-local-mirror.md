# 관리자 로컬 미러 — 관리자 화면이 읽는 데이터의 브라우저 사본 (1~8단계)
...
- 전체 재구축 주기는 7일(배경 유지보수가 돌린다). 「전체 새로 읽기」로 언제든 강제.
...
표의 첫 열은 **사용처** — 이 사본이 받치는 관리자 메뉴다. 재구축할지 말지는 「이게
낡으면 어느 화면이 틀리나」로 판단하는데, `feedPosts`·`boardPosts` 같은 컬렉션 이름은
Firestore 의 어휘지 화면의 어휘가 아니라 매번 코드를 뒤져야 했다. 문구는 `admin.html`
의 탭·사이드바와 글자까지 같게 유지한다 — 이 이름으로 화면을 찾아가는 것이 용도라,
다르게 적으면 오히려 헤매게 된다. 매핑은 `mirror-console.js` 의 `MIRROR_MENUS` 한 곳.
...
「7일마다 전체 재구축」을 모든 미러에 일괄로 걸어 두었는데, 미러마다 필요가 다르다.
축이 완전한 미러에게 정기 재구축은 잡을 것 없이 전량을 다시 사는 낭비다.

| 미러 | 정책 | 근거 |
|---|---|---|
| usageDaily | **정기 재구축 없음** | 모든 쓰기가 `updatedAt` 서버 시각을 찍는다 — 수정이 전부 델타에 걸린다. 삭제는 규칙이 막는다 |
| aiDietReports | **정기 재구축 없음** | `allow write: if false` — 생성 뒤 수정·삭제가 일어나지 않는다 |
| users | 7일 재구축 | `saveSettings` 가 아무 도장도 찍지 않는다(`setDoc merge` 뿐) — 로그인 없는 프로필 수정은 어떤 축으로도 못 잡는다 |
| sharedPhotos·feedPosts·boardPosts | 7일 재구축 | 생성 축 — 남이 고친 값(좋아요·댓글 수)이 델타에 안 걸린다 |
| meals | **드리프트 감지** | 재구축이 비싸(1.2만) 대신 동기화마다 count 1읽기로 대조한다 |

정기 재구축을 끈 미러도 **삭제 감지(count)는 남긴다** — Firebase 콘솔에서 손으로 지우는
경우의 마지막 안전망이고, 그때는 count 가 줄어 스스로 재구축한다.

**meals 드리프트 감지.** 건강한 미러는 항상 `미러 수 ≤ 서버 수` 다(미러는 date 있는
문서만 담는다). 미러가 더 크면 서버에서 사라진 문서를 미러가 들고 있다는 확실한 신호 —
툼스톤 유실이다. 걸리면 미러 콘솔 상태에 「정합성 의심 · 재구축 권장」 붉은 뱃지가 뜨고,
재구축은 1.2만 읽기라 **관리자가 콘솔에서 결정한다**(자동으로 돌리지 않는다).
못 잡는 것: 서버 수가 같은 채 내용만 어긋난 경우 — 그런 쓰기는 관리자 조치뿐이고
`patchLocalMeal` 이 그 자리에서 반영한다.

**배경 유지보수.** 7일 재구축은 원래 「그 화면을 다음에 열 때」만 돌았다. 자주 안 여는
화면의 미러는 기한이 지나도 낡은 채였고, 어쩌다 열면 그 자리에서 전체 재구축이 돌아
뜸금없이 느렸다. 이제 관리자 인증 직후와 6시간 간격으로 `runMirrorMaintenance` 가
구축된 미러들의 `ensureSynced` 를 배경에서 불러 각자 판단(델타/전체)에 맡긴다.
**미구축 미러는 건드리지 않는다** — 부트스트랩은 콘솔에서 사람이 부른다.
...
**화면 진입 자동 로드 (9단계).** 탭에 들어갈 때마다 「새로고침」을 눌러야 했던 것은
2026-04-01(`06ad572`)의 비용 가드다 — 그때는 진입이 곧 서버 전량 스캔이었다. 미러가
생긴 지금 진입 비용은 델타(count 1읽기 + 바뀐 문서 몇 건)라 근거가 사라졌으므로,
`mirror-autoload.js` 가 **구축된 미러 위에서만** 화면을 자동으로 채운다.

- 미구축 미러는 **건드리지 않는다** — 화면을 여는 것만으로 부트스트랩(meals 1.2만)을
  사면 미러로 없앤 비용이 그대로 돌아온다. 배경 유지보수와 **같은 게이트**(`bootstrapDone`)다.
  그때는 예전처럼 비어 있고 「새로고침」이 사람의 손으로 부른다.
- **세션당 1회**만 — 사이드바를 오갈 때마다 델타를 되사지 않는다. 관리자 조치는 그 자리에서
  미러와 화면에 함께 반영되므로 오가는 사이에 목록이 낡지 않는다.
- 못 채웠으면(미구축·실패) 표시를 지워 **다음 진입에 다시 시도**한다 — 미러 콘솔에서
  구축하고 돌아오면 그때는 채워져야 하기 때문이다.
- 대시보드는 원래도 자동이었다. 집계가 아니라 캐시 문서(`adminSettings/dashboardStats`)를
  1읽기 하는 길이라 부담이 없었다. 사용자 분석도 서브메뉴 전환이 이미 부르고 있었다.
...
| `js/admin/mirror-autoload.js` | 화면 진입 자동 로드 게이트 — 구축된 미러 위에서만 |
...
- **meals**: 미러에는 주기적 전체 재구축이 없다(부트스트랩 1회 + 델타·툼스톤). 그래서
  ...
  삭제 유실은 드리프트 감지(8단계)가 잡아 콘솔에 알리고, 「재구축 예약」이 마지막 수단이다.
