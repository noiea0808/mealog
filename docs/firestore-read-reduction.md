...
- **폴백 스캔 상한** — 모니터링 > 피드의 하루기록 목록은 정렬 인덱스가 없으면 순서 없이
  조회하는 폴백을 타는데, 한때 그 경로에만 `limit` 이 없어 인덱스 배포가 밀리면 조용히 전량
  스캔이 됐다. 지금은 목록이 sharedPhotos 조회로 재편되면서 모든 폴백 쿼리가
  `ADMIN_DAILY_JOURNAL_ROWS_CAP` 으로 잘린다 (`js/admin/feed-moderation.js`).
