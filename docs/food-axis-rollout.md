...
옛 축 1,900건을 재분류해 보니 1,197건(상세 텍스트가 있는 것) 중 66건이 사전에 없어 남았다.
그중 **47건**을 [food-dictionary.js](../js/utils/food-dictionary.js) 에 넣었다 — 실입력에서
확인된 이름만 넣었고, 짧은 접미('까스'·'카츠'·'국시'·'탕면')는 최장 일치가 기존 항목을
먼저 잡으므로 안전하다. `'스프'` 를 넣으면서 `'스프라이트'` 를 차/음료에 함께 둔 것이 그 예다.

남은 19건은 **음식명이 아니라 넣지 않았다**: 상호명(존스페이버릿·담원장모밥상·아뜰리에포요),
메모('4시 점심 겸 저녁?'·'공복몸무게56.9'·'다양'·'루틴식'), 그리고 **뷔페**. 뷔페는 무엇을
먹었는지 알려주지 않으므로 형태를 지어내는 대신 '미입력' 으로 두는 편이 정직하다.
