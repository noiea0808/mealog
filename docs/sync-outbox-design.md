...
| 로컬 읽기 실패 | **`[]` 가 아니라 `null`** (2026-08-21) | 아래 §5.1 |
| `permanent` 해제 | 사용자의 재전송 탭이 곧 「개입」 (2026-08-21) | 아래 §5.2 |

### 5.1 「못 읽었다」와 「비었다」를 구분한다

프로덕션 실측(2026-08-21)에서 항목 1건이 **앱을 다시 깔아도 사라지지 않는** 상태가 관측됐다.
진단 로그에는 `outbox.hydrate {"n":1}` 이 찍혀 있는데 `worker.cycle` 은 **한 번도** 없었다.

원인은 재시도 로직이 아니라 **읽기 실패의 표현**이었다. `idb.js` 의 `tx()` 는 데드라인(3초)에
걸리면 `null` 을 돌려주고 `getAll()` 이 그것을 `[]` 로 바꿨다. 그래서 `listPending` 이
「큐가 비었다」로 답했고 워커는 그대로 쉬었다. 배지는 부팅 때 채운 **메모리 인덱스**를 보므로
N 이 그대로 떠 있었고, 사용자가 FAB 을 눌러도 같은 빈 배열을 받아 아무 시도도 하지 않았다.

같은 날 로그에 `deadline {"label":"idb-tx:getAll:entries","ms":3000}` 이 6회, 그리고 다른
데이터베이스인 `idb-tx:putMany:events` 도 같은 시각에 한 번 찍혔다 — **스토리지 정체는
아웃박스만의 문제가 아니라 기기 전체에서 온다.** 그러므로 「가끔 못 읽는다」는 전제로 설계해야 한다.

- `getAllOrNull` 신설. `listPending` / `hydrateOutboxIndex` 는 실패를 `null` / `-1` 로 알린다.
- 워커는 못 읽은 사이클에서 **아무 결론도 내리지 않는다** (`worker.cycle.listFailed`).
- hydrate 실패 시 인덱스를 **비우지 않고** 백오프 재시도한다. 비우면 미전송 기록이
  화면에서 「반영됨」으로 보이고, 그보다 나쁘게는 `tick` 이 할 일 없음으로 판단해 워커가 멈춘다.
- `tick` 은 6초마다 스토어를 통째로 읽던 것을 **메모리 인덱스 조회**로 바꿨다. 폴링 자체가
  정체의 원인 중 하나였고, 인덱스는 배지가 이미 쓰는 단일 기준이라 같은 답을 공짜로 준다.
- **저장소가 아예 없는 경우**(시크릿 모드 등)는 여전히 `[]` 다 — 담긴 것도 없으므로 「비었다」가
  참이고, 재시도해도 소용이 없다. `null` 은 「스토어는 있는데 이번 읽기를 못 했다」만 뜻한다.

### 5.2 `permanent` 는 종착 상태가 아니다

§4.4 는 `permanent` 를 「개입 필요」로 정의했지만 **개입할 수단이 코드에 없었다.** 워커는
permanent 를 거르고, 배지는 그것까지 세고, 콘텐츠 등급은 만료도 없어서 항목이 영원히 갇혔다.
게다가 `permission-denied` 8회 승격은 **원인이 일시적인데도**(App Check·토큰 흔들림) 굳는다.

사용자가 재전송 FAB 을 직접 누른 것이 그 개입이다. `manual-resend` 사이클만 permanent 항목을
포함하고 `clearPermanent` 로 표식을 푼다. **`attempts` 는 보존한다** — 함께 리셋하면 백오프
시계가 0 으로 돌아가 6초 틱마다 재시도하는 뜨거운 루프가 된다. 다시 실패하면 워커가 그 자리에서
도로 permanent 로 찍으므로, 이 완화는 사용자가 누른 횟수만큼만 비용이 든다.

§4.5 의 보호는 그대로다: permanent 항목은 반드시 `attempts >= 1` 이므로 충돌 사전 확인
(`needsConflictCheck`)을 항상 타고, 서버가 더 최신이면 밀지 않고 제거된다.
