...
구현: `functions/spam-filter.js` (계약은 `test/spam-filter.test.mjs` 가 못박는다)

- **금칙어**: 광고, 홍보, 추천인 / 욕설, 비방, 혐오
  - `무료`·`이벤트`·`할인`·`쿠폰` 은 뺐다. 스팸의 표지가 아니라 그냥 쓰는 말이라
    「무료로 쓰실 수 있어요」 같은 정상 답변이 막혔다. 이 목록은 **그 말이 들어간 글이
    거의 다 스팸**일 때만 값을 한다.
  - 차단 시 **어떤 말이 걸렸는지 사용자에게 알려준다**. 예전에는 "금칙어가 포함되어 있습니다."만
    돌려줘서 사용자가 자기 글의 어디를 고쳐야 하는지 알 수 없었다.
- **과도한 외부 링크**: 외부 링크가 **2개를 넘을 때**(3개 이상) 차단.
  우리 도메인(`mealog.net`·`mealog.app`·`staging-mealog.vercel.app`·`play.google.com`)은 세지 않는다.

> 주의 — 정규식에 `/g` 를 붙이지 말 것. 목록은 모듈 스코프라 인스턴스의 모든 요청이 같은
> 객체를 공유하는데, `test()` 가 `lastIndex` 를 남겨 같은 글이 「차단 → 차단 → 통과」로
> 순환한다. 2026-08-30 에 실제로 그 상태였다: 멀쩡한 글이 무작위로 막히는 동시에,
> 전송 버튼을 세 번 누르면 스팸이 그냥 통과했다.
